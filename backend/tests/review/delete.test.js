...
describe('DELETE /api/review/id', () => {
    ...
    beforeEach(async () => {
        ...
        reviewId = createReviewRes.get("Location").split("/")[3] // the id 
    });


    it ("should return not found", async () => {
        const res = await request(app)
            .delete('/api/review/000000000000000000000000') // not found
            .set('Accept', 'application/json')
            .set('Authorization', `Bearer ${token}`)
    ...
    it ("should return 200", async () => {
        ...
        expect(user.body.rating.nbRating).toBe(0)
        expect(user.body.rating.punctualityRating).toBe(null)
        expect(user.body.rating.securityRating).toBe(null)
        expect(user.body.rating.comfortRating).toBe(null)
        expect(user.body.rating.courtesyRating).toBe(null)
    })
})